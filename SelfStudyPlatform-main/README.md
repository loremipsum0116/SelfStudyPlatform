...
- 서술형 입력창 우측의 **기호 버튼**으로 시그마/파이/거듭제곱/분수/근호 및 주요 논리·집합 기호를 입력할 수 있음
...
- 서술형 문항에는 **기호 셀렉터**가 붙어 있어 Σ/Π/거듭제곱/분수/근호를 값 채우기 방식으로 답안에 삽입할 수 있음
...
### 3-9. 오답노트 (`subjects/{과목ID}/wrong-notes/index.html`)
- 모의고사 오답만 과목별로 독립 저장
- 같은 문제를 다시 틀리면 **중복 생성하지 않고 기존 카드에 누적 갱신**
- 회색 배지 = 아직 복습 시점이 오지 않은 문제 수
- 빨간 배지 = 지금 다시 풀어야 하는 문제 수
- 숙달 상태까지 별도 관리하며, 오답노트 내부에서 다시 풀고 재채점 가능
...
`users/{uid}/subjects/{subjectId}/wrongNotes/{noteId}` — 문제 단위 누적 카드.

| 필드 | 설명 |
|---|---|
| `questionId` / `chapterId` | 원문 문제 식별자 |
| `questionType` | multiple-choice / short-answer / proof / truth-table |
| `questionText` / `choices` / `answer` / `explanation` | 오답노트 복습에 필요한 문제 스냅샷 |
| `wrongCount` | 누적 오답 횟수 |
| `reviewSuccessCount` | 복습 성공 누적 횟수 |
| `nextReviewAt` | 다음 복습 시각 |
| `status` | waiting / due / mastered 흐름의 기준 필드 |
| `lastUserAnswer` | 최근 입력 답안 |
| `lastWrongAt` / `lastReviewedAt` | 최근 오답 및 복습 시각 |

**저장 규칙**: 같은 과목 안에서 같은 문제를 다시 틀리면 새 문서를 만들지 않고 기존 오답노트를 갱신합니다.
